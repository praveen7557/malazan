...
const DuelPage: React.FC = () => {
  ...
  const getMarginColor = (margin: string) => {
    switch (margin) {
      case 'close': return 'text-yellow-600 dark:text-yellow-400';
      case 'decisive': return 'text-blue-600 dark:text-blue-400';
      case 'overwhelming': return 'text-red-600 dark:text-red-400';
  ...
  const getMarginBg = (margin: string) => {
    switch (margin) {
      case 'close': return 'bg-yellow-100 dark:bg-yellow-900/20';
      case 'decisive': return 'bg-blue-100 dark:bg-blue-900/20';
      case 'overwhelming': return 'bg-red-100 dark:bg-red-900/20';
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-red-50 to-orange-50 dark:from-gray-900 dark:to-red-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-300">Loading warriors...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-xl text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">
            Witness legendary battles between the most powerful beings in the Malazan universe
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border-l-4 border-blue-500">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Fighter 1</h2>
            ...
            {fighter1 && (
              <motion.div
                ...
                className="space-y-3"
              >
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{fighter1.name}</h3>
                <p className="text-sm text-gray-600 dark:text-gray-300">{fighter1.description}</p>
                
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex items-center gap-2">
                    <Zap className="w-4 h-4 text-red-500" />
                    <span className="text-sm">Power: {fighter1.powerLevel}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Wind className="w-4 h-4 text-blue-500" />
                    <span className="text-sm">Agility: {fighter1.agility}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Brain className="w-4 h-4 text-purple-500" />
                    <span className="text-sm">Intelligence: {fighter1.intelligence}</span>
                  </div>
                  <div className="flex items-center gap-2">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border-l-4 border-red-500">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Fighter 2</h2>
            ...
            {fighter2 && (
              <motion.div
                ...
                className="space-y-3"
              >
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{fighter2.name}</h3>
                <p className="text-sm text-gray-600 dark:text-gray-300">{fighter2.description}</p>
                
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex items-center gap-2">
                    <Zap className="w-4 h-4 text-red-500" />
                    <span className="text-sm">Power: {fighter2.powerLevel}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Wind className="w-4 h-4 text-blue-500" />
                    <span className="text-sm">Agility: {fighter2.agility}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Brain className="w-4 h-4 text-purple-500" />
                    <span className="text-sm">Intelligence: {fighter2.intelligence}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <TrendingUp className="w-4 h-4 text-green-500" />
          ...
          {isSimulating && (
            ...
            >
              <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8 max-w-md mx-auto">
                ...
                <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">Battle in Progress</h3>
                <p className="text-gray-600 dark:text-gray-300">The convergence begins...</p>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
          ...
          {duelResult && !isSimulating && (
            ...
            >
              <div className="text-center mb-6">
                ...
                <div className={`inline-block px-4 py-2 rounded-full text-sm font-medium ${getMarginBg(duelResult.margin)} ${getMarginColor(duelResult.margin)}`}>
                  {duelResult.margin.charAt(0).toUpperCase() + duelResult.margin.slice(1)} Victory
                </div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div className={`p-4 rounded-lg ${duelResult.winner._id === fighter1?._id ? 'bg-green-50 dark:bg-green-900/20 border-2 border-green-400' : 'bg-gray-50 dark:bg-gray-700'}`}>
                  <h3 className="font-semibold text-gray-900 dark:text-white mb-2">{fighter1?.name}</h3>
                  <div className="text-2xl font-bold text-gray-900 dark:text-white">
                    {fighter1?._id === duelResult.winner._id ? duelResult.winnerScore : duelResult.loserScore}
                  </div>
                </div>
                
                <div className={`p-4 rounded-lg ${duelResult.winner._id === fighter2?._id ? 'bg-green-50 dark:bg-green-900/20 border-2 border-green-400' : 'bg-gray-50 dark:bg-gray-700'}`}>
                  <h3 className="font-semibold text-gray-900 dark:text-white mb-2">{fighter2?.name}</h3>
                  <div className="text-2xl font-bold text-gray-900 dark:text-white">
                    {fighter2?._id === duelResult.winner._id ? duelResult.winnerScore : duelResult.loserScore}
                  </div>
                </div>
              </div>

              <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-3">Battle Chronicle</h3>
              ...
              <AnimatePresence>
                {showComparison && (
                  ...
                  >
                    <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
                      <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Statistical Comparison</h3>
                      ...
                      <div className="space-y-4">
                        ...
                        <div>
                          <div className="flex justify-between items-center mb-2">
                            ...
                            <div className="flex gap-4">
                              <span className="text-sm">{fighter1?.powerLevel}</span>
                              <span className="text-sm">{fighter2?.powerLevel}</span>
                            </div>
                          </div>
                          <div className="flex gap-2">
                            <div className="flex-1 bg-gray-200 dark:bg-gray-600 rounded-full h-2">
                              <div 
                                ...
                                style={{ width: `${(fighter1?.powerLevel || 0) / 100 * 100}%` }}
                              ></div>
                            </div>
                            <div className="flex-1 bg-gray-200 dark:bg-gray-600 rounded-full h-2">
                              <div 
                                className="bg-red-500 h-2 rounded-full transition-all duration-1000"
                                style={{ width: `${(fighter2?.powerLevel || 0) / 100 * 100}%` }}
                              ></div>
                            </div>
                          </div>
                        </div>
                        ...
                        <div>
                          <div className="flex justify-between items-center mb-2">
                            <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Agility</span>
                            <div className="flex gap-4">
                              <span className="text-sm">{fighter1?.agility}</span>
                              <span className="text-sm">{fighter2?.agility}</span>
                            </div>
                          </div>
                          <div className="flex gap-2">
                            <div className="flex-1 bg-gray-200 dark:bg-gray-600 rounded-full h-2">
                              <div 
                                className="bg-blue-500 h-2 rounded-full transition-all duration-1000"
                                style={{ width: `${(fighter1?.agility || 0) / 100 * 100}%` }}
                              ></div>
                            </div>
                            <div className="flex-1 bg-gray-200 dark:bg-gray-600 rounded-full h-2">
                              <div 
                                className="bg-red-500 h-2 rounded-full transition-all duration-1000"
                                style={{ width: `${(fighter2?.agility || 0) / 100 * 100}%` }}
                              ></div>
                            </div>
                          </div>
                        </div>
                        ...
                        <div>
                          <div className="flex justify-between items-center mb-2">
                            ...
                            <div className="flex gap-4">
                              <span className="text-sm">{fighter1?.intelligence}</span>
                              <span className="text-sm">{fighter2?.intelligence}</span>
                            </div>
                          </div>
                          <div className="flex gap-2">
                            <div className="flex-1 bg-gray-200 dark:bg-gray-600 rounded-full h-2">
                              <div 
                                className="bg-blue-500 h-2 rounded-full transition-all duration-1000"
                                style={{ width: `${(fighter1?.intelligence || 0) / 100 * 100}%` }}
                              ></div>
                            </div>
                            <div className="flex-1 bg-gray-200 dark:bg-gray-600 rounded-full h-2">
                              <div 
                                className="bg-red-500 h-2 rounded-full transition-all duration-1000"
                                style={{ width: `${(fighter2?.intelligence || 0) / 100 * 100}%` }}
                              ></div>
                            </div>
                          </div>
                        </div>
